    ...
    public async Task FunctionHandler(S3Event evnt, ILambdaContext context)
    {
        ...
        foreach (var record in eventRecords)
        {
            ...
            try
            {
                ...
                // in case we update in the same bucket that lambda trigger listening to 
                // so we have to mark that object (file) is already resized
                // if we not do this. it will make infinite loop of resizing same image
                ...
                {
                    image.Mutate(x => x.Resize(100, 100, KnownResamplers.Lanczos3));
